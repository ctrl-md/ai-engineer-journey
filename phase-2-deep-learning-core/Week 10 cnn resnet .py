...
if __name__ == "__main__":
    model = CNN(in_channels=1, hidden_channels=8, kernel_size=3, padding=1, num_classes=3)
    ...
    loss = out.sum()
    loss.backward()
    print(f"conv_in weight grad is non-zero: {model.conv_in.weight.grad.abs().sum().item() > 0}")
    print(f"resBlock conv1 weight grad is non-zero: {model.resBlock.conv1.weight.grad.abs().sum().item() > 0}")