...
class GPT(nn.Module):
    def __init__(self, vocab_size, d_model, max_seq_len, num_heads, d_ff, num_layers):
        super().__init__()
        self.token_embeddings = nn.Embedding(vocab_size, d_model)
        self.positional_embeddings = nn.Embedding(max_seq_len, d_model)
        self.blocks = nn.ModuleList(
            [Transformer(d_model, num_heads, d_ff) for _ in range(num_layers)]
        )
...
def data_split(x, y):
    ...
    return (
        x[:train_index],
        y[:train_index],
        x[train_index:val_index],
        y[train_index:val_index],
        x[val_index:],
        y[val_index:],
    )
...
def train(
    ...
    gpt = GPT(vocab_size, d_model, max_seq_len, num_heads, d_ff, num_layers)
    loss = nn.CrossEntropyLoss()
    optimizer = optim.Adam(
        gpt.parameters(), lr=learning_rate, weight_decay=weight_decay
    )
    ...
    for epoch in range(epochs):
        ...
        with no_grad():
            ...
            val_acc = val_acc.float().mean().item()
...
def evaluate(x_test, y_test, batch_size, gpt, vocab_size):
    ...
    for batch_num, (batch_x, batch_y) in enumerate(loader):
        ...
        test_acc = test_acc.float().mean().item()
...
if __name__ == "__main__":
    ...
    windows = [
        ...
        for i in range(0, len(all_tokens) - seq_len - 1, 4)
    ]
    ...
    test_performance = evaluate(
        x_test, y_test, batch_size=8, gpt=gpt, vocab_size=vocab_size
    )
