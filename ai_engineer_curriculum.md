...
**Phase 6 complete** — experiment tracking connected to the checkpointing pattern used throughout the course, DVC's pointer-plus-storage idea tied directly to `state_dict()` checkpointing, model serving worked through with real batching/latency numbers, distillation and pruning, the three ML-specific software engineering practices, and the phase deliverable: a real FastAPI serving app with genuine monitoring — verified end to end with a live server, including a real drift-detection catch on an out-of-distribution input the model itself was wrongly 99.9997% confident about. See `phase-6-mlops/`.

Currently on: Phase 7 — Research Skills.
